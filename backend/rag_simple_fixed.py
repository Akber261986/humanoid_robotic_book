"""
Simple RAG pipeline that bypasses TF-IDF vectorizer issues
"""
import os
...
def initialize_services():
    ...
    if QDRANT_API_KEY:
        ...
        logger.info(f"Qdrant client initialized with cloud URL: {QDRANT_URL}")
    else:
...
def create_simple_embedding(text: str, vocab_size: int = 1000) -> List[float]:
    """
    Create a simple embedding using basic word counting approach
    This creates a compatible embedding for query matching
    """
    # Simple approach: create a vector based on character n-grams
    # This is a simplified approach that creates a fixed-size vector
    import hashlib
    ...
    # Hash each token and distribute across the embedding vector
    for token in tokens:
        hash_val = int(hashlib.md5(token.encode()).hexdigest(), 16)
        idx = hash_val % vocab_size
        embedding[idx] += 1.0  # Simple count
...
def query_rag_pipeline(query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Step 1: Create simple embedding for the query
        logger.info(f"Creating simple embedding for query: {query[:50]}...")
...
def check_services():
    ...
    try:
        # Check if we can connect to Qdrant cloud
        try:
            if QDRANT_API_KEY:
                qdrant_client = QdrantClient(url=QDRANT_URL, api_key=QDRANT_API_KEY)
            else:
        ...
        # Check if TF-IDF vectorizer would be accessible (just checking if file exists)
        tfidf_ok = True  # Always true since we're using simple embedding now
